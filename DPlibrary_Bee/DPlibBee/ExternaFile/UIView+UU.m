...
{
    UIView* shbView = [[[self alloc] initWithFrame:frame] autorelease];
    if(shbView)
    {
        // 设置图层相关
        ...
        if(backgroundImage)
        {
            UIImageView* backgroundImageView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, shbView.frame.size.width, shbView.frame.size.height)] autorelease];
